...
public class JwtService {
    ...
    public String createAccessToken(String email) { // AccessToken 생성 메서드
        Date now = new Date();
        return JWT.create() // JWT 토큰을 생성하는 빌더 반환
                .withSubject(ACCESS_TOKEN_SUBJECT) // JWT의 Subject 지정 -> AccessToken이라서  AccessToken
                .withExpiresAt(new Date(now.getTime() + accessTokenExpirationPeriod)) // 토큰 만료 시간 설정

                //클레임으로 email 하나만 사용하고
                //추가는 .withClaim(클래임 이름, 클래임 값) 으로 설정하기
                .withClaim(EMAIL_CLAIM, email)
                .sign(Algorithm.HMAC512(secretKey)); // HMAC512 사용, application-jwt.yml에서 지정한 secret 키로 암호화
    }

    public String createRefreshToken() { // RefreshToken 생성 메서드 ( 리프레쉬 claim에 email도 넣을 필요 x)
        Date now = new Date();
        return JWT.create()
    ...
    //헤더에서 AccessToken 추출 후 토큰형식으로 BEARER 제거 후 순수 토큰만 가져오기 위해 헤더 갖고온 뒤 "BEARER 삭제 로직("" 로 만들어버림)
    public Optional<String> extractAccessToken(HttpServletRequest request) {
        return Optional.ofNullable(request.getHeader(accessHeader))
                .filter(refreshToken -> refreshToken.startsWith(BEARER))
                .map(refreshToken -> refreshToken.replace(BEARER, ""));
    }
    //AccessToken에서 Email 추출, 추출 전에 JWT.require()로 검증기 생성,
    // verify로 AceessToken 검증 후 유효하다면 getClaim()으로 이메일 추출하고 아니면 빈 Optional 객체 반환
