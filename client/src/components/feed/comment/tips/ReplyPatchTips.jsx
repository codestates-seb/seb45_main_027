...
const ReplyPatch = ({
  ...
  const tipCommentId = comment.tipCommentId;
  ...
  const patchReply = async () => {
    const configParams = {
      method: "PATCH",
      url: `/tip/${tipId}/tipReply/${commentId}/tipComment/${tipCommentId}`,
      headers: {
    ...
    try {
      ...
      if (response && response.status === 200) {
        ...
        updatedFeedData.replies.forEach((reply) => {
          if (reply.tipCommentId === tipCommentId) {
            reply.comments = reply.comments.map((comment) => {
              if (comment.tipCommentId === commentId) {
                // 새로운 객체 생성하여 내용 수정
  ...
  return (
    <span className="my-4 text-base">
      {editReply[comment.tipCommentId] ? (
        <>
          <div className="w-full flex">
            ...
            <button
              ...
              onClick={() => {
                patchReply();
                setEditReply({ ...editReply, [comment.tipCommentId]: false });
              }}>
              완료
            </button>
            <button
